...
export async function handlePremiumSelection(
  ...
  try {
    ...
    await bot.sendMessage(chatId, cardInfo);
...
export async function handleAdminApprove(
  ...
  try {
    ...
    if (!request) {
      await bot.answerCallbackQuery(chatId.toString(), { text: 'Request not found' });
      return;
    }
    
    const updated = await updatePremiumRequest(requestId, { status: 'approved' });
    ...
    if (updated) {
      ...
      await bot.sendMessage(chatId, `✅ Premium request approved for ${request.user_email}`);
    }
  } catch (error) {
  ...
  try {
    ...
    if (updated) {
      ...
      if (user) {
        await bot.sendMessage(
          request.user_telegram_id,
          t('premium_rejected', user.language, { reason: reason || 'Not specified' })
        );
      }
      
      await bot.sendMessage(chatId, `❌ Premium request rejected for ${request.user_email}`);
    }
  } catch (error) {
